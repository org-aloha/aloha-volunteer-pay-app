...
def get_sheet_client():
    # gspread標準の関数でSecretsから直接認証情報を読み込み
    return gspread.service_account_from_dict(dict(st.secrets["gcp_json"]))
...
if project_type == "対面mtg":
    project_name = st.text_input(
        "企画名", key=f"p_name_{st.session_state.reset_counter}"
    )
elif project_type == "その他":
    project_name = st.text_input(
        "企画名（内容）", key=f"p_name_{st.session_state.reset_counter}"
    )

combined_project = (
    f"{project_name}{project_type}" if project_name else project_type
)
...
for i in range(st.session_state.recipient_count):
    ...
    with col1:
        r_name = st.text_input(
            f"対象者名 {i+1}",
            key=f"r_name_{st.session_state.reset_counter}_{i}",
        )
    ...
    with col2:
        if needs_days:
            days = st.number_input(
                ...
                min_value=1,
                step=1,
                key=f"r_days_{st.session_state.reset_counter}_{i}",
            )
            days_str = str(days)
...
if not st.session_state.show_confirm:
    if st.button("完了", type="primary"):
        if (
            project_type in ["対面mtg", "その他"]
        ) and not project_name.strip():
            st.error(
                f"「{project_type}」が選択されています。企画名を入力してください。"
            )
        elif not dates:
    ...
    st.warning(
        "⚠️ 執行部メンバーは謝礼対象者に含まれません。このまま完了してもよろしいでしょうか。"
    )
        ...
        if st.button("OK (申請を確定する)", type="primary"):
            with st.spinner("スプレッドシートに保存中..."):
                try:
                    ...
                    sheet_1.update(
                        range_name=f"A{next_row_1}", values=rows_for_sheet_1
                    )
                    ...
                    rows_for_sheet_2 = []
                    for r in recipients_data:
                        # 14列分（要素数14）のリストを作成（インデックス0〜13）
                        row2 = [""] * 14
                        row2[0] = now_str  # 1列目: タイムスタンプ
                        row2[2] = r["name"]  # 3列目: スタッフ名
                        row2[3] = "法法人格"  # 4列目: 立て替え内容
                    ...
                    sheet_2.update(
                        range_name=f"A{next_row_2}", values=rows_for_sheet_2
                    )
